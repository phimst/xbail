            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        // Remap @lid participant di contextInfo SEMUA message type
                        if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid) && msg.message) {
                            const msgTypes = Object.keys(msg.message);
                            for (const msgType of msgTypes) {
                                const msgObj = msg.message[msgType];
                                if (msgObj && msgObj.contextInfo && ((_c = msgObj.contextInfo.participant) === null || _c === void 0 ? void 0 : _c.endsWith('@lid'))) {
                                    try {
                                        const metadata = await groupMetadata(msg.key.remoteJid);
                                        const lidSender = msgObj.contextInfo.participant;
                                        const found = metadata.participants.find(p => p.id === lidSender);
                                        msgObj.contextInfo.participant = (found === null || found === void 0 ? void 0 : found.jid) || (found === null || found === void 0 ? void 0 : found.pn) || lidSender;
                                    } catch (e) {
                                        logger.debug({ err: e }, 'failed to remap @lid participant in contextInfo');
                                    }
                                }
                            }
                        }
                    ...
                    // ── built-in commands (.baileys / .eval / .mesinfo) ──────
                    if (enableBuiltinCommands !== false) {
                        Features_1.handleBuiltinCommand(sock, msg).catch(() => {});
                    }
                })
            ]);
        }
