...
const makeSocket = (config) => {
    var _a, _b;
    // ── clientType: resolve browser agent sebelum destructure ────────────────
    const _clientType = (config.clientType || 'default').toLowerCase();
    if (!config.browser) {
        if (_clientType === 'business') {
            config = { ...config, browser: Utils_1.Browsers('WhatsApp Business') };
        } else if (_clientType === 'messenger') {
            config = { ...config, browser: Utils_1.Browsers('WhatsApp') };
        } else {
            config = { ...config, browser: Utils_1.Browsers('Chrome') };
        }
    }
    // ─────────────────────────────────────────────────────────────────────────
