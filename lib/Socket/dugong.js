...
class kikyy {
    ...
    async handleGroupStory(content, jid, quoted) {
        ...
        const msgId = this.bail.generateMessageID();
        const msgContent = {
        ...
        // wrap agar return object punya .key seperti message lain
        const fullMsg = await this.utils.generateWAMessageFromContent(jid, msgContent, {});
    ...
    async sendStatusWhatsApp(content, jids = []) {
        ...
        for (const id of jids) {
            try {
                ...
                // skip JID yang bukan user maupun group
                if (!isPrivate && !isGroup) continue;
                ...
                // delay natural antar mention agar tidak dikira spam
                await Utils_1.delay(1500 + Math.floor(Math.random() * 1000));
            } catch (error) {
                // log tapi jangan stop loop — lanjut ke JID berikutnya
                this.config.logger.warn({ id, err: error.message }, 'sendStatusMention: skip jid due to error');
                continue;
            }
        }
